...
export function generateFallback(meta) {
  ...
  if(tp.includes("객관식")) {
    ...
    return {passage:null,stem:p.q,type:"mc",options:opts,answer:String(ci),explanation:p.ex,visual:p.v||null};
    ...
    return {passage:null,stem:`"${stem}의 답은 ${shown}이다." 이 문장이 맞으면 O, 틀리면 X를 선택하세요.`,type:"ox",options:null,answer:isCorrect?"O":"X",explanation:`정답은 ${ans}이므로 ${shown === ans ? "맞습니다" : "틀립니다"}. ${p.ex}`,visual:p.v||null};
  } else if(tp.includes("빈칸")) {
    return {passage:null,stem:p.q.replace("구하시오","빈칸에 알맞은 수를 써넣으시오").replace("무엇입니까","얼마입니까"),type:"fill",options:null,answer:String(ans),explanation:p.ex,visual:p.v||null};
  } else {
    return {passage:null,stem:p.q.replace("구하시오","풀이 과정을 자세히 쓰고 답을 구하시오"),type:"essay",options:null,answer:"essay",explanation:p.ex,visual:p.v||null};
  }
}
