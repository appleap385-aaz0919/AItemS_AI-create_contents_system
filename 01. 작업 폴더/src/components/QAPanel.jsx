      ...
      <div className="qa-grid">
        {[["D1 코드",qaResult.D1_code],["D2 교육",qaResult.D2_education],["D3 접근성",qaResult.D3_accessibility],["D4 채점",qaResult.D4_scoring]].map(([n,d],i)=>(
          <div key={i} className="qa-domain">
            ...
            {d?.issues?.length>0&&<div className="qa-di">{(Array.isArray(d.issues)?d.issues:[]).map(v=>typeof v==="string"?v:(v?.description||v?.detail||v?.message||v?.text||"")).filter(Boolean).join("; ")}</div>}
          </div>
        ))}
      </div>
