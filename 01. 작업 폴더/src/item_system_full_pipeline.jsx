...
async function _callClaudeInner(sys, user, callId) {
  for(let attempt=0; attempt<2; attempt++){
    ...
    addLog("시도 " + (attempt+1) + "/2 — fetch 시작...");
    try {
      ...
      if(r.status === 429) {
        addLog("⚠️ 429 Rate Limit — 3초 대기");
        if(attempt<1){ await new Promise(r=>setTimeout(r,3000)); continue; }
        return null;
      }
      if(!r.ok) {
        const errBody = await r.text().catch(()=>"");
        addLog("❌ HTTP 에러 " + r.status + ": " + errBody.substring(0,100));
      ...
      if(d.error) {
        addLog("❌ API 에러: " + (d.error?.message || JSON.stringify(d.error)).substring(0,100));
        if(attempt<1){ await new Promise(r=>setTimeout(r,1500)); continue; }
        return null;
      }
  ...
  if(callId === _apiCallId) addLog("❌ 2회 모두 실패");
  return null;
}

// 폴백 문항 생성 (API 실패 시) — 학습맵 기반 풍부한 문항
function generateFallback(meta) {
  const tp = meta.tp || "객관식(4지선다)";
  const d3 = meta.depth?.d3?.name || "";
  const d2 = meta.depth?.d2?.name || "";

  // 문제 유형별 다양한 템플릿
  const problems = [
    // 덧셈
    ()=>{ const a=Math.floor(Math.random()*400)+100,b=Math.floor(Math.random()*400)+100; return {q:`${a} + ${b}의 값을 구하시오.`,a:a+b,ex:`${a} + ${b} = ${a+b}입니다. 일의 자리부터 차례로 더합니다.${(a%10+b%10)>=10?" 일의 자리에서 받아올림이 발생합니다.":""}`}; },
    // 뺄셈
    ()=>{ const a=Math.floor(Math.random()*400)+200,b=Math.floor(Math.random()*199)+100; return {q:`${a} - ${b}의 값을 구하시오.`,a:a-b,ex:`${a} - ${b} = ${a-b}입니다. 일의 자리부터 차례로 뺍니다.${(a%10<b%10)?" 일의 자리에서 받아내림이 필요합니다.":""}`}; },
    // 어림셈
    ()=>{ const a=Math.floor(Math.random()*400)+100,b=Math.floor(Math.random()*400)+100,ra=Math.round(a/100)*100,rb=Math.round(b/100)*100; return {q:`${a} + ${b}를 백의 자리에서 반올림하여 어림한 값을 구하시오.`,a:ra+rb,ex:`${a} → 약 ${ra}, ${b} → 약 ${rb}이므로, 어림한 합은 ${ra} + ${rb} = ${ra+rb}입니다.`}; },
    // 곱셈 기초
    ()=>{ const a=Math.floor(Math.random()*8)+2,b=Math.floor(Math.random()*8)+2; return {q:`${a} × ${b}의 값을 구하시오.`,a:a*b,ex:`${a}을(를) ${b}번 더하면 ${a} × ${b} = ${a*b}입니다.`}; },
    // 나눗셈 기초
    ()=>{ const b=Math.floor(Math.random()*7)+2,a=b*(Math.floor(Math.random()*8)+2); return {q:`${a} ÷ ${b}의 값을 구하시오.`,a:a/b,ex:`${a}을(를) ${b}씩 묶으면 ${a/b}묶음이 됩니다. ${a} ÷ ${b} = ${a/b}`}; },
    // 자릿값
    ()=>{ const n=Math.floor(Math.random()*900)+100,d=Math.floor(Math.random()*3),pos=["일","십","백"][d],val=Math.floor(n/Math.pow(10,d))%10; return {q:`${n}에서 ${pos}의 자리 숫자는 무엇입니까?`,a:val,ex:`${n}에서 ${pos}의 자리 숫자는 ${val}입니다.`}; },
    // 비교
    ()=>{ const a=Math.floor(Math.random()*400)+100,b=a+Math.floor(Math.random()*50)-25; return {q:`${a}와 ${b} 중 더 큰 수는 무엇입니까?`,a:Math.max(a,b),ex:`${a}와 ${b}를 비교하면 ${Math.max(a,b)} > ${Math.min(a,b)}이므로, ${Math.max(a,b)}가 더 큰 수입니다.`}; },
  ];
...
export default function App() {
  ...
  useEffect(()=>{
    checkApiStatus();
    const iv=setInterval(checkApiStatus, 30000);
  ...
  const runFullPipeline=async(m)=>{
    console.log("[Pipeline] ===== 파이프라인 시작 =====", m?.tp);
    ...
    console.log("[Pipeline] TEAM 01 기획팀 시작");
    await new Promise(r=>setTimeout(r,500));
    updateStage(0,"done","0.5s");
    console.log("[Pipeline] TEAM 01 기획팀 완료");
    ...
    while(attempt<MAX_RETRY&&!qaOk){
      attempt++;
      console.log("[Pipeline] === 제작+검수 시도", attempt, "/", MAX_RETRY, "===");

      // TEAM 02: 제작팀
      updateStage(1,"running");
      addLog("[제작팀] API 호출 시작...");
      const t0 = Date.now();
      try {
        if(attempt>1){
      ...
      addLog("[제작팀] API 응답: " + (Date.now()-t0) + "ms " + (q?"✅ OK":"❌ FAIL"));
      let usedFallback = false;
      if(!q) {
        q = generateFallback(m);
        usedFallback = true;
        addLog("[제작팀] ⚠️ 로컬 문항으로 대체");
      }
      setQuestion(q);
      const genTime = Date.now()-t0;
      updateStage(1,"done",usedFallback?"로컬 생성":`${attempt>1?attempt+"차 재저작":"AI 생성 완료"} (${Math.round(genTime/1000)}s)`);

      // ★ 제작팀 완료 후 건너뛰기 플래그 반드시 초기화 (검수팀에 영향 안 줌)
      _apiSkip = false;
      skipRef.current = false;

      // TEAM 03: 검수팀 (항상 실행 — 절대 건너뛰지 않음)
      updateStage(2,"running");
      const t1 = Date.now();
      if(usedFallback) {
        // 제작팀 API 실패 → 같은 서버이므로 검수 API도 불가 → 로컬 검수
        addLog("[검수팀] 제작팀 API 실패 → 로컬 규칙 검수");
        qa = localValidate(m,q);
      } else {
        addLog("[검수팀] API 호출 시작...");
        try {
          qa=await apiValidate(m,q);
        } catch(e) {
          console.error("[Pipeline] 검수팀 에러:", e);
          qa = localValidate(m,q);
        }
        if(!qa) qa = localValidate(m,q);
      }
      addLog("[검수팀] 결과: " + (Date.now()-t1) + "ms " + qa?.score + "점 " + (qa?.status==="approved"?"✅ 승인":"❌ 반려") + (qa?._localValidation?" (로컬)":""));
      setQaResult(qa);
      qaOk=qa.status==="approved";
      updateStage(2,qaOk?"done":"fail",
        `${qa.score}점 ${qaOk?"승인":`반려(${attempt}/${MAX_RETRY})`}${qa._localValidation?" (로컬 검수)":""}`);
    ...
    if(!qaOk){
      ...
      // ★ ESCALATED여도 문항은 존재 → 타이머 시작 + 데이터팀 진행
    }
      ...
      <div className="lay">
        ...
        <div className="mn">
          ...
          {selNode&&<>
            {!showTypeSelect&&<>
              {/* API 상태 표시등 */}
              <div className="api-status-bar" onClick={checkApiStatus} title="클릭하여 수동 체크">
                ...
                <span className="api-status-text">
                  {apiStatus.checking?"체크 중...":
                   ...
                   apiStatus.state==="ratelimit"?"API 요청 제한 (잠시 대기)":
            ...
            {/* API 연결 테스트 버튼 */}
            <div style={{padding:"8px 0",display:"flex",gap:"6px",flexWrap:"wrap"}}>
              <button className="bsub" style={{fontSize:".65rem",padding:"6px 12px",background:"#6c5ce7"}} onClick={async()=>{
                ...
                try {
                  const r = await fetch("https://api.anthropic.com/v1/messages", {
                    method:"POST", headers:{"Content-Type":"application/json"},
                    body: JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:50, messages:[{role:"user",content:"1+1=?"}] })
                  });
                  const ms=Date.now()-t0;
                  ...
                  if(r.ok){
                    ...
                    addLog("❌ HTTP " + r.status + ": " + body.substring(0,100));
                  }
                } catch(e) {
                  addLog("❌ " + e.name + ": " + e.message);
