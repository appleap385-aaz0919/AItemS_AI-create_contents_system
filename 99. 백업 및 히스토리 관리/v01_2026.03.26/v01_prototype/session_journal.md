...
| 파일 | 설명 |
|---|---|
| `item_system_full_pipeline.jsx` | 프로토타입 본체 (React JSX, ~2,600줄) |
...
## 사용자 컨텍스트 (대화에서 파악한 사항)
- 사용자는 Claude를 **"PM"**이라고 부름
- HTML 변환은 요청 시에만 (매번 만들 필요 없음)
- 노션 문서화를 보고용 + 개발자 참고용으로 활용하려 함
- 백업/버전 관리를 중시 — 중간중간 백업본을 만들어 롤백 가능하게
- 현재 버전: **v01_prototype** (backups/v01_prototype.zip 에 백업됨)
...
### 12. forceOX 버그 수정
- **버그**: 서술형 선택했는데 OX 문제 출제
- **원인**: genRightAngle()의 forceOX:true가 사용자 유형 선택 덮어씌움
- **수정**: forceOX 속성 + 강제변환 로직 완전 제거

---

## 시도했으나 실패한 접근 (재시도 금지)

### ZIP 다운로드 반복 문제 — Artifact sandbox 한계
| 시도 | 방식 | 결과 |
|---|---|---|
| 1차 | blob + a.click() | 첫 1회만 동작, 2회차부터 조용히 차단 |
| 2차 | data:base64 URI | sandbox에서 data: 다운로드 차단 |
| 3차 | React state <a> 렌더링 | DOM 재사용으로 2회차 차단 |
| 4차 | key={Date.now()} 강제 재생성 | 여전히 차단 |
| 5차 | 숨겨진 child iframe 격리 | iframe 접근 자체가 sandbox에서 차단 |
| **결론** | **가장 단순한 blob + a.click()** | **1회 보장이 최선, 코드로 우회 불가** |
...
| 버전 | 날짜 | 설명 | 백업 파일 |
|---|---|---|---|
| v01_prototype | 2026-03-26 | 프로토타입 초기 완성 | backups/v01_prototype.zip |
